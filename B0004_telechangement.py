...
# ===================================================================
# Spécifier les SPV qui seront traités
# ===================================================================
nom_SPV = []
...
for key_projet,row_projet in df_projet_lien.iterrows(): # parcourir les projet
    if row_projet["SPV"] in nom_SPV: # est ce que il est dans un SPV à traiter
        for lienNom in dic_lien_chemin.keys(): # Parcourir la liste des pdf possibles
            if not pd.isna(row_projet[lienNom]): # si ce projey contiens ce pdf
                lien_pdf_Wrike = row_projet[lienNom] # recuprer la lien de pdf dans Wrike
                nom_pdf_wrik = row_projet["Nom_" + lienNom[4:]] # recuprer le nom de pdf dans Wrike
                cheminPDF, nomPDF = dic_lien_chemin[lienNom] # recuprer le chemin relatif et le nom de pdf dans dataroom
                # get le chemin absolu de pdf dans dataroom
                chemin_absolu_pdf_dataroom= path_stokage + r"/" + row_projet["SPV"] +r"/Projets" + r"/" + row_projet['nom projet'] 
                chemin_absolu_pdf_dataroom = chemin_absolu_pdf_dataroom +  cheminPDF
                # get le nom de pdf dans dataroom
                nom_pdf_dataroom =   row_projet['nom projet'] +"-"+nomPDF +row_projet[lienNom][-4:]
                # get le createTime de pdf dans wrike
                createTime_pdf = time.mktime(time.strptime(row_projet["createdDate_" + lienNom[4:]], '%Y-%m-%dT%H:%M:%SZ'))
                list_telecharge.append([chemin_absolu_pdf_dataroom,
                                        nom_pdf_dataroom,
                                        lien_pdf_Wrike,
                                        createTime_pdf,
                                        index_tele,
...
df_list_telecharge = pd.DataFrame(columns=["chemin_absolu_pdf_dataroom",
                                           "nom_pdf_dataroom",
                                           "lien_pdf_Wrike",
                                           "createTime_pdf",
                                           "index_tele",
                                           "nom_pdf_wrik"],data=list_telecharge)
